...
describe('MoviesController (e2e)', () => {
    ...
    const mockMovieModel = {
        ...
        find: jest.fn().mockReturnValue({
            ...
            exec: jest.fn().mockResolvedValue(
                [{ ...mockMovie, _id: mockMovie._id.toString() }].filter(movie => !deletedMovieIds.has(movie._id))
            ),
        }),
        ...
        findOneAndUpdate: jest.fn().mockImplementation((query, update) => {
            ...
            const updatedMovie = update
                ? { ...mockMovie, ...update.$set, _id: mockMovie._id.toString(), __v: query.__v + 1 }
                : null;
    ...
    it('/movies (GET) - should return a list of movies', async () => {
        ...
        expect(res.body.data).toEqual([{ ...mockMovie, _id: mockMovie._id.toString() }]);
    });

    it('/movies/:id (GET) - should find a movie by ID', async () => {
        const res = await request(app.getHttpServer()).get(`/movies/${mockMovie._id}`).expect(200);
    ...
    it('/movies/:id (PUT) - should update a movie by ID', async () => {
        const res = await request(app.getHttpServer())
            ...
            .expect(200);
        expect(res.body).toEqual({ ...updatedMovie, _id: mockMovie._id.toString() });
    });
    ...
    it('/movies/:id (PATCH) - should update the runtime of a movie by ID', async () => {
        ...
        const expectedMovie = { ...mockMovie, runtime: updatedRuntime, __v: clientVersion + 1 };
        expect(res.body).toEqual({ ...expectedMovie, _id: mockMovie._id.toString() });
    });
